...
import { IDebtToken, IERC20, IOracle, IFlashSwap, ZeroInterestMarket, ZeroInterestMarket__factory } from "../typechain";
    ...
    context("post-construction", () => {
        ...
        describe("deposit", () => {
            it("deposits collateral for user", async () => {
                ...
                await expect(market.connect(borrower).deposit(borrower.address, AMOUNT))
                    ...
                    .withArgs(borrower.address, borrower.address, AMOUNT);
            ...
            it("can deposit collateral to another user", async () => {
                ...
                await expect(market.connect(borrower).deposit(other.address, AMOUNT))
                    .to.emit(market, "Deposit")
                    .withArgs(borrower.address, other.address, AMOUNT);
        ...
        describe("borrow", () => {
            beforeEach(async () => {
                ...
                await expect(market.connect(borrower).deposit(borrower.address, COLL_AMOUNT))
                    .to.emit(market, "Deposit")
                    .withArgs(borrower.address, borrower.address, COLL_AMOUNT);
            });
            ...
            it("borrow event emitted", async () => {
                ...
                await expect(market.connect(borrower).borrow(borrower.address, BORROW_AMOUNT))
                    ...
                    .withArgs(borrower.address, borrower.address, TOTAL_DEBT);
            });
            ...
            it("reverts if LTV is too high", async () => {
                ...
                debtToken.transfer.returns(true);
                await expect(market.connect(borrower).borrow(borrower.address, BORROW_AMOUNT)).to.be.revertedWith(
                    "Market: exceeds Loan-to-Value"
                );
        ...
        describe("withdraw", () => {
            ...
            beforeEach(async () => {
                ...
                await expect(market.connect(borrower).borrow(borrower.address, BORROW_AMOUNT))
                    .to.emit(market, "Borrow")
                    .withArgs(borrower.address, borrower.address, DEBT_AMOUNT);
            ...
            it("emits event", async () => {
                await expect(market.connect(borrower).withdraw(borrower.address, `1${E18}`))
                    ...
                    .withArgs(borrower.address, borrower.address, `1${E18}`);
            });
            ...
            it("reverts if reducing LTV below limit", async () => {
                await expect(market.connect(borrower).withdraw(borrower.address, `9${E18}`)).to.be.revertedWith(
                    "Market: exceeds Loan-to-Value"
                );
            ...
            it("withdrawal exceeds collateral balance", async () => {
                await expect(market.connect(borrower).withdraw(borrower.address, `11${E18}`)).to.be.revertedWith(
                    "Market: amount too large"
                );
            });

            it("reverts if LTV is exceeded (bounds check)", async () => {
                await expect(
                    market.connect(borrower).withdraw(borrower.address, `1541900000000000000`)
        ...
        describe("repay", () => {
            ...
            it("cannot repay more than owed debt", async () => {
                await expect(
                    market.connect(borrower).repay(borrower.address, `507500000000000000001`)
                ).to.be.revertedWith("");
            });
            ...
            it("can repay another users' debt", async () => {
                await expect(market.connect(other).repay(borrower.address, DEBT_AMOUNT))
                    .to.emit(market, "Repay")
                    .withArgs(other.address, borrower.address, DEBT_AMOUNT);
        ...
        describe("repayAndWithdraw", () => {
            it("is a convenience wrapper for the two functions", async () => {
                ...
                await expect(market.connect(borrower).depositAndBorrow(`10${E18}`, `300${E18}`))
                    .to.emit(market, "Deposit")
                    .withArgs(borrower.address, borrower.address, `10${E18}`)
                    .and.emit(market, "Borrow")
                    .withArgs(borrower.address, borrower.address, `304500000000000000000`);
                await expect(market.connect(borrower).repayAndWithdraw(`304500000000000000000`, `10${E18}`))
                    .to.emit(market, "Repay")
                    .withArgs(borrower.address, borrower.address, `304500000000000000000`)
                    .and.emit(market, "Withdraw")
                    .withArgs(borrower.address, borrower.address, `10${E18}`);
        ...
        describe("repayAndWithdraw - bad oracle", () => {
            it("test with bad oracle", async () => {
                ...
                await expect(
                    market.connect(borrower).repayAndWithdraw(`304500000000000000000`, `10${E18}`)
        ...
        describe("liquidate", () => {
            ...
            it("will claim collateral from underwater users", async () => {
                ...
                await expect(
                    market
                        .connect(liquidator)
                        .liquidate(borrower.address, DEBT_AMOUNT, 0, liquidator.address, ethers.constants.AddressZero)
                )
                    .to.emit(market, "Liquidate")
                    .withArgs(borrower.address, liquidator.address, DEBT_AMOUNT, collateralLiquidated, PRICE)
                    .and.emit(market, "Repay")
                    .withArgs(liquidator.address, borrower.address, DEBT_AMOUNT)
                    .and.emit(market, "Withdraw")
                    .withArgs(borrower.address, liquidator.address, collateralLiquidated);
            ...
            it("can us a swapper to obtain debt tokens from collateral on open market", async () => {
                ...
                expect(swapper.swap).to.be.calledWith(
                    collateralToken.address,
                    debtToken.address,
                    liquidator.address,
                    ethers.BigNumber.from(DEBT_AMOUNT),
                    ethers.BigNumber.from(collateralLiquidated)
                );
                expect(collateralToken.transfer).to.be.calledWith(liquidator.address, collateralLiquidated);
                expect(debtToken.transferFrom).to.be.calledWith(liquidator.address, market.address, DEBT_AMOUNT);
            });
            ...
            it("can partially claim collateral from underwater users", async () => {
                ...
                await market
                    .connect(liquidator)
                    .liquidate(borrower.address, REPAY_AMOUNT, 0, liquidator.address, ethers.constants.AddressZero);
            ...
            it("allows liquidator to buy some of the collateral even if worth less than the debt", async () => {
                ...
                await market
                    .connect(liquidator)
                    .liquidate(borrower.address, `5${E18}`, 0, liquidator.address, ethers.constants.AddressZero);
            ...
            it("allows liquidator to revert if they don't get as much collateral as they expect", async () => {
                const DEBT_AMOUNT = "507500000000000000000"; // $507.50
                await market.connect(borrower).borrow(borrower.address, `500${E18}`);
                expect(await market.userDebt(borrower.address)).to.equal(DEBT_AMOUNT);
                ...
                // liquidation penalty 10%, so price collateral at $72
                // $507.50 worth of collateral at $72 is 7.0486
                const collateralLiquidated = `704861111111111112`;
                ...
                ).be.revertedWith("excess collateral slippage");
            });
            ...
            it("cannot liquidate themself", async () => {
                await expect(
                    market
                        .connect(liquidator)
                        .liquidate(liquidator.address, 1, 0, liquidator.address, ethers.constants.AddressZero)
            ...
            it("can send claimed collateral to another user", async () => {
                ...
                await expect(
                    market
                        .connect(liquidator)
                        .liquidate(borrower.address, DEBT_AMOUNT, 0, other.address, ethers.constants.AddressZero)
                )
                    .to.emit(market, "Liquidate")
                    .withArgs(borrower.address, other.address, DEBT_AMOUNT, collateralLiquidated, PRICE)
                    .and.emit(market, "Repay")
                    .withArgs(liquidator.address, borrower.address, DEBT_AMOUNT)
                    .and.emit(market, "Withdraw")
                    .withArgs(borrower.address, other.address, collateralLiquidated);
        ...
        describe("Frozen market - bad oracle price", () => {
            it("no good oracle price", async () => {
                ...
                await expect(
                    market
                        .connect(liquidator)
                        .liquidate(borrower.address, AMOUNT, 0, liquidator.address, ethers.constants.AddressZero)
                ).to.be.revertedWith("Market: frozen");

                await expect(market.connect(borrower).borrow(borrower.address, AMOUNT)).to.be.revertedWith(
                    "Market: frozen"
                );
            });
        });
        ...
        describe("reduceSupply", () => {
            ...
            it("can only be called by the owner", async () => {
                await expect(market.connect(other).reduceSupply(`1000${E18}`)).to.be.revertedWith(
        ...
        describe("setTreasury", () => {
            ...
            it("can only be done by the owner", async () => {
                await expect(market.connect(other).setTreasury(other.address)).to.be.revertedWith(
                    "Ownable: caller is not the owner"
                );
            });

            it("cannot be set to a zero address", async () => {
                await expect(market.connect(owner).setTreasury(ethers.constants.AddressZero)).to.be.revertedWith(
                    "Market: 0x0 treasury address"
                );
            });
        });
        ...
        describe("emergency pause", () => {
            ...
            it("emits an event", async () => {
                await expect(market.connect(owner).setPaused(true)).to.emit(market, "Paused").withArgs(owner.address);
            });

            it("can only be done by the owner", async () => {
                await expect(market.connect(other).setPaused(true)).to.be.revertedWith(
                    "Ownable: caller is not the owner"
                );
            });
            ...
            it("ensure critical functionality is paused", async () => {
                ...
                await expect(market.connect(borrower).deposit(borrower.address, AMOUNT)).to.be.revertedWith(
                    "Pausable: paused"
                );
                await expect(market.connect(borrower).withdraw(borrower.address, AMOUNT)).to.be.revertedWith(
                    "Pausable: paused"
                );
                await expect(market.connect(borrower).repay(borrower.address, AMOUNT)).to.be.revertedWith(
                    "Pausable: paused"
                );
                await expect(market.connect(borrower).borrow(borrower.address, AMOUNT)).to.be.revertedWith(
                    "Pausable: paused"
        ...
        describe("recoverERC20", () => {
            ...
            it("cannot recover debt token", async () => {
                await expect(market.connect(owner).recoverERC20(debtToken.address, 100)).to.be.revertedWith(
                    "Cannot recover debt tokens"
                );
            });

            it("cannot recover collateral token", async () => {
                await expect(market.connect(owner).recoverERC20(collateralToken.address, 100)).to.be.revertedWith(
                    "Cannot recover collateral tokens"
                );
            });

            it("can only be done by the owner", async () => {
                await expect(market.connect(other).recoverERC20(collateralToken.address, 100)).to.be.revertedWith(
                    "Ownable: caller is not the owner"
                );
            });
        });
