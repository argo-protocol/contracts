...
describe("MarketFactory", () => {
    ...
    describe("createZeroInterestMarket", () => {
        it("reverts if signer is non-owner", async () => {
            await expect(marketFactory.connect(other).createZeroInterestMarket(example.address, example.address, example.address, example.address, 1, 2, 3)).to.be.reverted;
        });

        it("creates initialized market and emits event", async () => {
            for (let i = 0; i < 3; i++) {
                const ltv = 1 + i;
                await expect(marketFactory.connect(owner).createZeroInterestMarket(example.address, example.address, example.address, example.address, ltv, 2, 3))
        ...
        it("sets ownership to marketFactory owner", async () => {
            marketFactory.connect(owner).createZeroInterestMarket(example.address, example.address, example.address, example.address, 1, 2, 3);
